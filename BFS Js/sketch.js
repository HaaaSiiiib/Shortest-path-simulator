...
{	
	...
	{
		is_height = true;
		var cnv = createCanvas(windowHeight/1.1,windowHeight/1.1);
	}
	else 
	{
		is_height = false;
		var cnv = createCanvas(windowWidth/1.1,windowWidth/1.1);
	}
...
{
  let count = 0;
  let open_nodes = new Queue();
  let open_nodes_track = new Set();
  let closed_nodes_track = new Set();
  running = true;
  open_nodes_track.add(start.id);
  open_nodes.enqueue(start.id);
  closed_nodes_track.add(start.id);
  let parent_node = {};
  let current;
  while(!open_nodes.isEmpty())
  {
    ...
    for (let current_neighbor in current_neighbors)
    {
      ...
      if (closed_nodes_track.has(neighbor.id))continue;
      else
      {
        parent_node[neighbor.id] = current.id;
        open_nodes.enqueue(neighbor.id);
        if (neighbor!=end)neighbor.make_open();
      }
      
      if (keyboardActions())return false;    
    }
    closed_nodes_track.add(current.id);
    if (current != start)
    {
      current.make_close();
    }
